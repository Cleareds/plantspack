...
export async function GET(request: NextRequest) {
  try {
    ...
    if (level === 'places' && country && !city) {
      ...
      let query = supabase
        .from('places')
        .select('id, slug, name, category, subcategory, address, description, images, main_image_url, average_rating, review_count, is_pet_friendly, vegan_level, website, phone, opening_hours, latitude, longitude, city, country, cuisine_types, verification_level, verification_method, last_verified_at, is_verified, source, created_by, tags')
        .is('archived_at', null)
        .ilike('country', fromSlug(country))
    ...
    if (level === 'places' && country && city) {
      ...
      const selectCols = 'id, slug, name, category, subcategory, address, description, images, main_image_url, average_rating, review_count, is_pet_friendly, vegan_level, website, phone, opening_hours, latitude, longitude, city, country, cuisine_types, verification_level, verification_method, last_verified_at, is_verified, source, created_by, tags'
      const all: any[] = []
      const BATCH = 1000
      let from = 0
      while (true) {
