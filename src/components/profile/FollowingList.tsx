...
export default function FollowingList({ className = '' }: FollowingListProps) {
  ...
  if (!user) {
    return (
      <div className={`text-center py-8 ${className}`}>
        <p className="text-outline">Please log in to see who you&apos;re following.</p>
      </div>
    )
  }
  ...
  if (loading) {
    return (
      <div className={`space-y-4 ${className}`}>
        {[...Array(3)].map((_, i) => (
          <div key={i} className="flex items-center space-x-3 p-4 bg-surface-container-lowest rounded-lg border animate-pulse">
  ...
  if (following.length === 0) {
    return (
      <div className={`text-center py-8 ${className}`}>
        ...
        <h3 className="text-lg font-medium text-on-surface mb-2">No one yet</h3>
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      {following.map((user) => (
        <div key={user.id} className="flex items-center justify-between p-4 bg-surface-container-lowest rounded-lg border hover:bg-surface-container-low transition-colors">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              {user.bio && (
                <p className="text-sm text-on-surface-variant mt-1 line-clamp-2">{user.bio}</p>
              )}
            </div>
          </div>
          ...
          <button
            ...
            className="flex items-center space-x-1 px-3 py-1 text-sm text-error hover:bg-error/5 rounded-md transition-colors"
            title="Unfollow"
          >
