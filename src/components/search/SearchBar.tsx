...
export default function SearchBar({ className = '' }: SearchBarProps) {
  ...
  const { cities, countries, places, loading } = useSearch(query)
  ...
  const hasResults = cities.length > 0 || countries.length > 0 || places.length > 0
        ...
        <div className="sm:min-w-[420px] absolute top-full left-0 right-0 mt-1 bg-surface-container-lowest glass-float shadow-ambient rounded-lg z-50 max-h-[28rem] overflow-y-auto">
          ...
          {showNoResults && (
            <div className="p-4 text-center">
              ...
              <Link href="/map" onClick={handleResultClick}
            ...
            <div className="divide-y divide-outline-variant/10">
              ...
              {(cities.length > 0 || countries.length > 0) && (
                <div>
                  ...
                  {countries.map(c => (
                    <Link key={`country-${c.slug}`} href={`/vegan-places/${c.slug}`} onClick={handleResultClick}
                  ...
                  {cities.map(c => (
                    <Link key={`city-${c.slug}-${c.country}`} href={`/vegan-places/${c.country.toLowerCase().replace(/\s+/g, '-')}/${c.slug}`} onClick={handleResultClick}
                      className="flex items-center gap-3 px-3 py-2.5 hover:bg-surface-container-low transition-colors">
              ...
              {places.length > 0 && (
                <div>
                  ...
                  {places.map(place => (
                    <Link key={place.id} href={`/place/${place.slug || place.id}`} onClick={handleResultClick}
                      className="flex items-center gap-3 px-3 py-2.5 hover:bg-surface-container-low transition-colors">
