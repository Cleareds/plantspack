-- Sub-grade tiers: add B+, C+, D+ between the existing letter grades.
-- The + tier sits at the top of each bucket (B+ = top 7 pts of B range,
-- C+ = top 8 pts of C range, D+ = top 7 pts of D range) so cities that
-- are clearly leading their bucket get visible credit without having to
-- jump to the next letter.
--
-- Updates the CASE inside the materialized view's SELECT. Can't do an
-- ALTER MATERIALIZED VIEW to change SELECT semantics, so we drop + recreate
-- — the `refresh_directory_views()` cron (daily) + on-mutation triggers
-- will repopulate it. Data is not lost; scores are recomputed from places.
