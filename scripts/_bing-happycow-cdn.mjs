...
const places = []
for (const c of ['Mexico','Argentina','Chile']) {
  let from = 0
  while (true) {
    const { data } = await sb.from('places').select('id,name,city,country')
...
async function findHcCdn(name, city, country) {
  ...
  for (const q of queries) {
    try {
      ...
      // Match any happycow CDN URL with size 1024/500/250
...
async function worker() {
  while (cursor < places.length) {
    ...
    if (found) {
      ...
      ok++
      if (ok <= 20 || ok % 10 === 0) console.log(`+ [${ok}] ${p.name} [${p.city}]: ${found.src.slice(60)}`)
    } else { miss++ }
  }
}
