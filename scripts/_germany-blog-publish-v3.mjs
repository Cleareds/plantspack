...
**Readers:** if a venue's hours, address, or vegan status here doesn't match what you find on the ground, every place page has a "Suggest correction" button. Corrections land in our admin queue and we review weekly.

**Venue owners:** if you run one of the places mentioned here and want to update photos, opening hours, menu links, or verify your 100% vegan status, email us at hello@plantspack.com with the place URL and what you'd like updated. Owner-confirmed listings are flagged as such on the public page.
...
*Last audited: May 2026. Plants Pack is free, ad-free, and supported by readers. [Support us for EUR 3/month](/support).*`
...
console.log(error ? `✗ ${error.message}` : `✓ Updated ${POST_ID}: ${content.length} chars, ${TITLE}`)
